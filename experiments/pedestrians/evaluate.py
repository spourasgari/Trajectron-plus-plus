...
def load_model(model_dir, env, ts=100):
    ...
    model_registrar.load_models(ts)             # Loading the model from --model path
    with open(os.path.join(model_dir, 'config.json'), 'r') as config_json:
        hyperparams = json.load(config_json)

    trajectron = Trajectron(model_registrar, hyperparams, None, 'cpu')

    trajectron.set_environment(env) # Setting the environment from --data path
    trajectron.set_annealing_params()
    return trajectron, hyperparams      # trajectron = Trajectron() : predictions_dict
...
if __name__ == "__main__":
    ...
    if 'override_attention_radius' in hyperparams:  # The distance to consider the agents as influential
    ...
    with torch.no_grad():
        ...
        for i, scene in enumerate(scenes):
            ...
            predictions, z = eval_stg.predict(scene,
                                           timesteps,
                                           ph,
                                           num_samples=1,
